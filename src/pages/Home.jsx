    ...
    <div className="bg-white text-gray-900 font-inter">
          ...
          {images.map((src, index) => (
            <SwiperSlide key={index}>
              <div className="relative w-full h-full">
                <img src={src} alt={`AGX Hero Banner ${index + 1}`} className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-black bg-opacity-40 flex flex-col justify-center items-center text-center px-4">
                  <h1 className="text-4xl md:text-5xl font-extrabold text-white mb-4 drop-shadow-2xl animate-fade-in-up">
      ...
      <section className="flex flex-col md:flex-row items-center justify-between py-16 px-4 animate-fade-in-up transition-opacity duration-700">
        ...
        <div className="w-full md:w-1/2">
          <img src={globalBusinessImage} alt="AGX About Section" className="rounded-md shadow-xl" />
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto px-4">
            {[1, 2, 3].map((num, idx) => (
              <div key={idx} className="bg-white shadow-md rounded-md p-6 hover:shadow-xl transition duration-500 transform hover:-translate-y-2 animate-fade-in-up">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">{t(`homepage.service${num}Title`)}</h3>
                <p className="text-gray-700">{t(`homepage.service${num}Desc`)}</p>
              </div>
            ))}
          </div>
          <div className="mt-10">
